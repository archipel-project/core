...
impl Window {
    pub fn new(window: winit::window::Window, wgpu_instance : wgpu::Instance) -> anyhow::Result<(Self, Context)> {
    ...
    fn get_surface_configuration(surface: &wgpu::Surface, size: winit::dpi::PhysicalSize<u32>, render_context : &Context) -> wgpu::SurfaceConfiguration {
    ...
    pub fn resize(&mut self, size: winit::dpi::PhysicalSize<u32>, render_context : &Context) {
        ...
        if size.width > 0 && size.height > 0 {
            self.surface.configure(&render_context.wgpu_device, &self.surface_config);
...
impl<'a> FrameRenderer<'a>{
    ...
    pub fn new(window: &'a Window, context: &'a Context) -> Result<FrameRenderer<'a>, wgpu::SurfaceError> {
        let (surface_texture, output_view) = Self::get_surface_texture(&window.surface)?;
        let depth_buffer = window.depth_buffer.create_view(&wgpu::TextureViewDescriptor::default());
        Ok(Self {
            context,
            surface_texture,
            output_view,
    ...
    fn get_command_encoder(&self) -> wgpu::CommandEncoder {
        self.context.wgpu_device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("render scheduler encoder") })
    }

    fn get_surface_texture(surface: &wgpu::Surface) -> Result<(wgpu::SurfaceTexture, wgpu::TextureView), wgpu::SurfaceError> {
    ...
    pub fn render<T>(self, job_list: T) where
        T: Tuple,
        <T as Tuple>::TupleList : RenderJob
    {
        ...
        self.context.wgpu_queue.submit(std::iter::once(command_encoder.finish()));
