...
impl PaletteChunk for Chunk8Bits {
    ...
    fn get_or_create_palette_index(&mut self, state : BlockState) -> Option<u8> {
        ...
        for i in 0..self.palette.len() {
            if self.palette[i] == 0 { //0 means empty and can be used
...
impl InMemoryChunk for Chunk4Bits {
    ...
    fn try_set_block(&mut self, pos: BlockPos, state: BlockState) -> bool {
        ...
        if let Some(palette_index) = get_or_create_palette_index {
            ...
            if is_first_half {
                self.blocks[array_index as usize] = (self.blocks[array_index as usize] & 0b11110000) | palette_index;
            } else {
                self.blocks[array_index as usize] = (self.blocks[array_index as usize] & 0b00001111) | (palette_index << 4);
            }
            return true;
        }
        false
