...
impl<DataObject> GuiHandler<DataObject>{
    pub fn new(window: &super::Window, graphic_context: &super::Context) -> Self {
        ...
        let state = egui_winit::State::new(context.clone(), context.viewport_id(), window.as_winit_window(), None, None);
        let renderer = egui_wgpu::Renderer::new(&graphic_context.wgpu_device, window.get_surface_config().format, Some(super::Window::DEPTH_FORMAT), 1);
    ...
    pub fn update_gui(&mut self, window: &super::Window, graphic_context: &super::Context, data_object: &mut DataObject) {
        ...
        let egui::FullOutput{
            ...
            pixels_per_point,
            ..
        } = self.context.run(raw_input, |ctx| self.gui_pointer.current_gui(ctx, data_object));
        ...
        self.draw_data = Some(DrawData{ clipped_primitives, screen_descriptor });
    }

}

impl<DataObject> RenderJob for GuiHandler<DataObject> {
    fn update(&mut self, command_encoder: &mut wgpu::CommandEncoder, graphic_context: &super::Context) {
        let draw_data = self.draw_data.as_ref().unwrap();
        self.renderer.update_buffers(&graphic_context.wgpu_device, &graphic_context.wgpu_queue, command_encoder, &draw_data.clipped_primitives, &draw_data.screen_descriptor);
    }

    fn draw<'pass>(&'pass mut self, render_pass: & mut wgpu::RenderPass<'pass>) {
        let draw_data = self.draw_data.as_ref().unwrap();
        self.renderer.render(render_pass, &draw_data.clipped_primitives, &draw_data.screen_descriptor);
    }
}
